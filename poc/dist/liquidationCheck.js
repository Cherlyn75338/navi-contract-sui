...
function computeSeized(debtToCover, debtPriceInt, debtPriceDec, collPriceInt, collPriceDec, bonusBps // e.g., 10500 for 5% bonus
) {
    const target = 18;
    // scale prices to target
    const adj = (v, d) => {
        if (d > target)
            return v / (10n ** BigInt(d - target));
        if (d < target)
            return v * (10n ** BigInt(target - d));
        return v;
    };
...
//# sourceMappingURL=liquidationCheck.js.map