...
async function devInspectValue(pkg, fn, args, module, typeArgs) {
...
async function fetchPrice(pkg, coinType) {
    ...
    if (ret.length === 0)
        throw new Error('price getter returned no values');
    const priceInt = BigInt(ret[0][0]);
    const priceDecimals = Number(ret[1]?.[0] || 9);
    return { priceInt, priceDecimals };
}
async function userValue(pkg, user, coinType, assetIndex, Dc, priceInt, Dp) {
...
//# sourceMappingURL=trackA.js.map