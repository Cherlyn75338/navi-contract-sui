...
async function main() {
    ...
    let incentiveObjs = [];
    try {
        poolObjs = await listPackageObjects(poolPkg);
    }
    catch (e) {
        console.log('Pool object enumeration not available:', e.message);
    }
    try {
        storageObjs = await listPackageObjects(storagePkg);
    }
    catch (e) {
        console.log('Storage object enumeration not available:', e.message);
    }
    if (incentivePkg) {
        try {
            incentiveObjs = await listPackageObjects(incentivePkg);
        }
        catch (e) {
            console.log('Incentive object enumeration not available:', e.message);
        }
    }
    ...
    if (ENV.ORACLE_ID)
        withdrawArgs.push(tx.object(ENV.ORACLE_ID));
    else if (oraclePkg) {
    ...
    withdrawArgs.push(tx.object(ENV.STORAGE_ID), tx.object(ENV.POOL_B_ID), tx.pure(Number(ENV.ASSET_ID)), tx.pure(1), tx.pure(ENV.SIGNER));
    if (incentives.length > 0)
        withdrawArgs.push(tx.object(incentives[0].data.objectId));
    if (incentivesV2.length > 0)
        withdrawArgs.push(tx.object(incentivesV2[0].data.objectId));
    tx.moveCall({
