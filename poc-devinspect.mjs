...
async function devInspectNoRepay() {
  ...
  const coinFromLoan = tx.moveCall({
    ...
    typeArguments: [TYPE_ARG],
    arguments: [{ kind: "NestedResult", index: 0, resultIndex: 0 }],
  });
  tx.moveCall({
  ...
  return client.devInspectTransactionBlock({ sender: SENDER, transactionBlock: await tx.build({ client }) });
}

async function devInspectWithRepay() {
  const tx = new TransactionBlock();
  tx.setSender(SENDER);
  const lr = tx.moveCall({
    target: `${LENDING_PKG}::lending::flash_loan_with_ctx`,
    typeArguments: [TYPE_ARG],
    arguments: [tx.object(CONFIG_ID), tx.object(POOL_ID), tx.pure.u64(AMOUNT)],
  });
  ...
  tx.moveCall({
    ...
    arguments: [
      ...
      tx.object(POOL_ID),
      { kind: "NestedResult", index: 1, resultIndex: 1 },
      loanBal,
    ],
  });
  return client.devInspectTransactionBlock({ sender: SENDER, transactionBlock: await tx.build({ client }) });
