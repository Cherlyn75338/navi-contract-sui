...
async function harvestFromFunction(module, func, max = 1000) {
  const filter = { MoveFunction: { package: PKG, module, function: func } };
  const pools = new Map();
  let cursor = null;
  let scanned = 0;
  while (scanned < max) {
    const res = await queryTxs(filter, cursor, 50);
...
(async () => {
  try {
    const funcs = [
      ...
      ['lending', 'deposit'],
      ['lending', 'withdraw'],
      ['storage', 'init_reserve'],
    ];
    const combined = new Map();
    for (const [m, f] of funcs) {
      const part = await harvestFromFunction(m, f, 1500);
      for (const [id, t] of part.entries()) combined.set(id, t);
    }
    const groups = new Map();
    for (const [id, t] of combined.entries()) {
    ...
    console.log(JSON.stringify({ pkg: PKG, totalPools: combined.size, coins: out, multiplicity }, null, 2));
  } catch (e) {
    console.error('ERR', e);
    process.exit(1);
  }
