...
async function findRecentSample(module: string, func: string) {
...
function extractLikelyObjects(objChanges: any[]) {
...
async function main() {
  ...
  for (const f of funcs) {
    ...
    for (const tx of txs) {
      const objCh = (tx as any).objectChanges || [];
      const e = extractLikelyObjects(objCh);
      const nonEmpty = Object.keys(e).filter((k) => (e as any)[k]?.length);
      if (nonEmpty.length) {
        console.log(`  tx ${tx.digest}`);
        for (const k of nonEmpty) {
          const arr = (e as any)[k];
          console.log(`    ${k}:`);
