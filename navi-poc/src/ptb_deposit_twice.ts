...
async function tryDepositTwice(assetIndex: number) {
  const tx = new Transaction();
  // Split two tiny SUI coins from gas (in MIST)
  const amt1 = 10_000_000n; // 0.01 SUI
  const amt2 = 10_000_000n;
...
async function main() {
  for (let idx = 0; idx < 64; idx++) {
    try {
      ...
      console.log(`assetIndex=${idx} error: ${(e as Error).message}`);
    }
  }
}
