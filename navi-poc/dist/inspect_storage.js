...
async function main() {
    ...
    for (const f of fields.data) {
        console.log(`nameType=${f.name?.type || "?"}`);
        console.log(`  nameVal=${JSON.stringify(f.name?.value)}`);
        console.log(`  objectId=${f.objectId}`);
        try {
            const o = await client.getObject({ id: f.objectId, options: { showType: true, showContent: true } });
            console.log(`  child type=${o.data?.type}`);
        }
        catch (e) {
            console.log(`  child fetch error: ${e.message}`);
        }
    }
}
main().catch((e) => {
...
//# sourceMappingURL=inspect_storage.js.map